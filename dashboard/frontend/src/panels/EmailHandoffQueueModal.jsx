...
// ── Active-prospect queue for the Email Handoff sequence ─────────────────────
// Same visual pattern as DmFollowUpQueueModal.jsx, backed by
// GET/POST/DELETE /api/dialer/email-handoff-active (routers/dialer.py).
// Dropping off this list on a real reply (out-of-office auto-replies don't
// count), unsubscribe, or after Touch 3 is automatic — the endpoint mirrors
// email_handoff_sequence.send_due_touches()'s own stop conditions.
...
function AddToEmailHandoffModal({ activeContactIds, onClose, onAdded }) {
  ...
  async function add(contact) {
    ...
    setErr("");
    const res = await fetch(`/api/dialer/email-handoff-active/${contact.id}`, { method: "POST" });
    if (res.ok) {
      onAdded();
      onClose();
    } else {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 70, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", background: "#0a1020", border: "0.5px solid #1a2540", borderRadius: 8,
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16, fontWeight: 700, color: "#f0f4ff" }}>Add Prospect to Email Handoff</div>
        ...
        <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 10, color: "#3a5a80" }}>
          Sets the contact to Email Handoff and starts the sequence from Touch 1 (sends 24h from now). Re-adding someone restarts their sequence. Needs an email on file; refused for anyone who unsubscribed.
        </div>
          ...
          {results.map(c => {
            ...
            return (
              <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px",
                ...
                <button
                  onClick={() => add(c)}
                  disabled={!c.email || already || addingId === c.id}
...
export default function EmailHandoffQueueModal({ onClose }) {
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/dialer/email-handoff-active");
      const data = await res.json();
  ...
  async function remove(row) {
    const name = row.owner || row.business || row.email;
    if (!window.confirm(`Remove ${name} from Email Handoff? No further touches will send unless someone re-adds them.`)) return;
    setBusyId(row.contact_id);
    setErr("");
    const res = await fetch(`/api/dialer/email-handoff-active/${row.contact_id}`, { method: "DELETE" });
    if (res.ok) fetchRows();
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ position: "absolute", inset: 0, background: "rgba(4,8,16,0.85)" }} onClick={onClose} />
      <div style={{ position: "relative", background: "#0a1020", border: "0.5px solid #1a2540", borderRadius: 8,
                    width: "min(1000px, 92vw)", maxHeight: "85vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "18px 24px", borderBottom: "0.5px solid #1a2540",
          ...
          <div>
            <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16, fontWeight: 700, color: "#f0f4ff" }}>
              Email Handoff — Active Prospects
            </div>
        ...
        <div style={{ margin: "10px 24px 0", fontSize: 11, color: "#3a5a80", flexShrink: 0 }}>
          Drops off this list automatically on a real reply (out-of-office auto-replies don't count), an unsubscribe, or once Touch 3 sends. "Remove" is for pulling someone out manually before that. Sends are throttled per mailbox, so a touch can go out a little after its due time.
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "14px 24px 20px" }}>
          {loading ? (
            ...
            <div style={{ padding: "30px 0", textAlign: "center", fontFamily: "'Share Tech Mono', monospace",
              ...
              NOBODY CURRENTLY ACTIVE IN THE EMAIL HANDOFF SEQUENCE
            </div>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {rows.map(r => (
                  <tr key={r.contact_id} style={{ borderBottom: "0.5px solid #121e36" }}>
