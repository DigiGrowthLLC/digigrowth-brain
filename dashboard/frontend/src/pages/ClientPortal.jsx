...
// ── Live sequence management — client-portal port of
// panels/SequenceQueueModal.jsx, backed by the client-branded engines
// (client_appointment_reminders.py / client_appointment_sequence.py) via
// /portal-api/{token}/sequences/{sequence}/... instead of
// /api/appointment-reminders/sequence/{sequence}/... — same three sequence
// keys ("reminder" | "no_show" | "cancel"), same shape, just token-scoped to
...
function AddToClientSequenceModal({ token, sequence, activeIds, onClose, onAdded }) {
  const copy = CLIENT_SEQUENCE_COPY[sequence];
...
function ClientSequenceQueueModal({ token, sequence, onClose }) {
  const copy = CLIENT_SEQUENCE_COPY[sequence];
  ...
  async function remove(row) {
    const name = row.prospect_name || row.owner || `#${row.id}`;
              ...
              <tbody>
                {rows.map(r => (
                  <tr key={r.id} style={{ borderBottom: "0.5px solid #121e36" }}>
                    <td style={{ padding: "8px 10px", fontSize: 12, color: "#f0f4ff" }}>{r.prospect_name || r.owner || "—"}</td>
                    <td style={{ padding: "8px 10px", fontSize: 11, color: "#7a94b8" }}>{r.step_label}</td>
                    <td style={{ padding: "8px 10px", fontSize: 11, color: "#7a94b8" }}>{fmtSeqDate(r.next_touch_due_at)}</td>
...
function SequencesTab({ token }) {
  ...
  const [loading, setLoading] = useState(true);
  const [openQueue, setOpenQueue] = useState(null); // "reminder" | "no_show" | "cancel" | null
  ...
  return (
    <div>
      <div className="glass-card" style={{ padding: "20px 24px", marginBottom: 24 }}>
        <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16, fontWeight: 700, color: "#f0f4ff", marginBottom: 6 }}>
          Your Patient Messaging
        </div>
        <div style={{ fontSize: 12.5, color: "#8aaad0", lineHeight: 1.6 }}>
          Your appointment reminders and follow-ups are live, sending from your own number and
          mailbox. Below is the copy for each, plus who's currently in each sequence.
        </div>
      </div>
      ...
      {SEQUENCE_ORDER.map((key) => {
        ...
        const modalKey = SEQUENCE_MODAL_KEY[key];
        return (
      ...
      {openQueue && (
        <ClientSequenceQueueModal token={token} sequence={openQueue} onClose={() => setOpenQueue(null)} />
      )}
    </div>
  );
}
