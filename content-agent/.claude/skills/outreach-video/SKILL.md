...
### STEP 8 — Report
Tell Dylan the watch URL (for texting) and the local file path (for reference/archival).

### Batch runs
When given multiple leads in one request, repeat Steps 2–8 **one prospect at a time, in the
foreground**. Do not background this work — unattended runs in this repo have silently died when
backgrounded before (see the leadgen scraping backgrounding incident). Report each watch URL as
it finishes rather than batching all reports to the end.
