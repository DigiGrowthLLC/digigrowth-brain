...
4. **Write the page.** This is your own reasoning, not a template fill — use the scraped text/colors and the reference file's guidance. The page always has exactly these six sections, in this order — this structure is fixed, not a suggestion:
   ...
   **Two specific mobile bugs already found and fixed once — don't reintroduce them:**
   - **Never give an image `height: 100%` inside a container that has no explicit height.** A percentage height needs a defined parent height to resolve against; without one, the image collapses to zero height and silently disappears — this happened to the hero photo in the very first real build. Give the image's container a real `aspect-ratio` (e.g. `aspect-ratio: 4/3`, adjusted per-breakpoint if the crop should change) instead of relying on a parent's auto height.
   - **Don't leave a `min-height` (e.g. `88svh` for an above-the-fold hero) unset on mobile once the layout stacks to one column.** The two-column desktop hero can reasonably want a tall minimum height; the stacked single-column mobile version doesn't need the same forced height and will show a large dead gap below the content if it's not reset (e.g. `min-height: auto`) in the mobile media query.
   - **Verify mobile with `add_style_tag(content: ".reveal{opacity:1 !important}")` before screenshotting**, not a raw full-page screenshot. A plain Playwright full-page screenshot does not scroll through the page, so the `.reveal` sections' `IntersectionObserver` never fires and everything below the first viewport renders as invisible (`opacity: 0`) — that's a screenshot-methodology artifact, not a real bug, and it's easy to misdiagnose as one. Force reveal-visibility before capturing to see the actual layout.
   - **Add a `@media (max-width: 480px)` pass that specifically shrinks every headline/subhead**, not just the hero's. Desktop/tablet heading sizes (28px section headers, 44px hero headline, etc.) are too large relative to a phone screen — they eat most of the viewport and read as "a lot to read" even when the copy itself is short. Bring every section's `h2`, its kicker, and its intro paragraph down a size at this breakpoint (roughly: hero headline to ~26px, section h2s to ~19-22px, body copy down a point or two) — see `references/example-landing-page.html`'s 480px block for the actual values that read well.
   - **Any inline icon/dot placed next to text that might wrap to multiple lines (e.g. the scarcity line's pulsing dot) needs to flow as inline content, not sit as a separate flex sibling.** `display:flex; align-items:center` on a dot+paragraph pair looks fine on one line, but centers the dot against the whole two-line block once the text wraps — it visually detaches from the text it's supposed to sit next to. Make the icon `display:inline-block; vertical-align:middle` inside the same text flow instead, so it wraps as part of the sentence.
