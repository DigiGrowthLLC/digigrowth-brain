...
| Folder | Description |
|---|---|
| `dashboard/` | DigiGrowth OS — React + FastAPI web app (CRM, dialer, SMS, analytics, agent chat). Deployed on Railway. |
...
| `parallel-dialer/` | Twilio-based parallel dialer (up to 10 simultaneous calls) |
...
- **Parallel Dialer** — `cd parallel-dialer && doppler run -- python run.py`
