...
- `design-agent/` — DigiGrowth's web-page/website builder: scrapes a named cold-outreach prospect's real site and builds a hosted, personalized landing-page mockup (`landing-page-lead-magnet` skill), served via `dashboard/backend/routers/landing_pages.py` + a `landing_pages` Postgres table (HTML stored directly) + Cloudflare R2 for the hero image
